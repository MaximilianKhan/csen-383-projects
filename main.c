...
// INIT TIME.
    ...
    int clock = 0, counter = 0, wait_time, result, nread, parent_flag = 0;
                    ...
                    for (int i = 0; i < CHILDREN; i++) {
                        if (FD_ISSET(fd[i][READ_END], &inputfds)) {
                            ...
                            if (nread == 0) {
                                ...
                                fprintf(fd_out, "%s", buffer); fflush(stdout);
                            }
                        }
                    }
                    break;
...
// CHILD PROCESSES: PASS TIMESTAMP
        ...
        timeDiff(
            ...
            &(today->tm_min), &(today->tm_sec), (int*)&(tv.tv_usec),
            &diff_min, &diff_sec, &diff_usec);
        sprintf(buffer, "%02d:%02d.%03d: Child %d DONE\n",
                diff_min,
                diff_sec,
                diff_usec / 1000,
                child);
